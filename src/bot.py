...
class Bot:
    ...
    def search_player(self, player, max_price):
        ...
        coins = self.driver.find_element(
            By.CLASS_NAME, 'view-navbar-currency-coins').text.replace(",", "")
        ...
        while int(coins) >= max_price and success_count < 5:
            ...
            if "DetailView" in result.get_attribute("class"):
                ...
                try:
                    self.driver.find_element(
                        By.XPATH, '//button[contains(@class, "buyButton")]').click()
                ...
                self.driver.find_element(
                    By.XPATH, '//div[contains(@class,"view-modal-container")]//button').click()
                ...
                if int(coins) == int(new_coins):
                    print("(x) Found something, but someone already got it (x)")
                else:
                    price = int(coins) - int(new_coins)
                    print("[Success! You got " + player +
                          " for " + str(price) + " coins.]")
                    coins = new_coins
                    success_count += 1
            ...
            try:
                self.driver.find_element(
                    By.XPATH, '//button[contains(@class, "ut-navigation-button-control")]').click()
            except WebDriverException:
                wait_for_shield_invisibility(self.driver, 0.1)
                self.driver.find_element(
                    By.XPATH, '//button[contains(@class, "ut-navigation-button-control")]').click()
    ...
    def buy_player(self, player, max_price):
        try:
            ...
            sleep(0.1)
            self.driver.find_element(
                By.XPATH, '//div[contains(@class, "ut-player-search-control")]//input').send_keys(player)
            ...
            self.driver.find_element(
                By.XPATH, '//ul[contains(@class, "playerResultsList")]/button').click()

            self.driver.find_element(
                By.XPATH, '(//input[@class="numericInput"])[4]').click()
            sleep(0.1)
            self.driver.find_element(
                By.XPATH, '(//input[@class="numericInput"])[4]').send_keys(max_price)

            print("Looking for " + player +
                  " with max price " + str(max_price) + "...")
